...
Get the proper WAR file (e.g. jbpm-dashbuilder-eap-6_3.war) and execute the following command:

    $ cd <jboss_home>/bin
    $ ./jboss-cli.sh --connect --command="deploy <path_to_war_file>"

    <path_to_war_file>: is the local path to the application war file.
    e.g. $ ./jboss-cli.sh --connect --command="deploy /home/myuser/myfiles/jbpm-dashbuilder-eap-6_3.war" )
...
* Modify the file *jbpm-dashboard/jbpm-dashboard-distributions/src/main/eap6_3/WEB-INF/jboss-web.xml*:
...
* Modify the file *jbpm-dashboard/jbpm-dashboard-distributions/src/main/eap6_3/WEB-INF/jboss-deployment-structure.xml*.
...
<code>eap-6_3/standalone/configuration/standalone.xml</code> file and add the <code>&lt;sso/&gt;</code> tag under the virtual server configuration.
...
So, in JBoss, we can just define them in the following file: <code>eap-6_3/standalone/configuration/standalone.xml</code>,
...
* First of all, in order to login as superuser, using the <code>eap-6_3/bin/add-user.sh</code> command utility,
...
* The application roles are defined at [jbpm-dashboard-distributions/src/main/eap6_3/WEB-INF/web.xml](https://github.com/droolsjbpm/jbpm-dashboard/blob/master/jbpm-dashboard-distributions/src/main/eap6_3/WEB-INF/web.xml) file.
Roles can be used to create access profiles and define custom authorization policies.

* The application uses the JBoss' default security domain as you can see [here](https://github.com/droolsjbpm/jbpm-dashboard/blob/master/jbpm-dashboard-distributions/src/main/eap6_3/WEB-INF/jboss-web.xml).
